...
<Expandable title="Weight Configuration">
  ...
  Weight for blast radius score (0-1). Currently active.
  </ParamField>
...
<Note>
Unavailable risk factors are not included in the scores object. Currently, only data_sensitivity, input_clarity, and blast_radius are active.
</Note>
...
Adjust weights based on your risk tolerance. Default weights:
- `data_sensitivity`: 0.20 (20%)
- `input_clarity`: 0.40 (40%)
- `blast_radius`: 0.40 (40%)
...
When you receive high-risk scores (&ge; 0.6):

1. **Review recommendations**: Check the `recommendations` field for the risk level assessment
2. **Consider manual review**: High scores (0.6-0.79) and critical scores (&ge; 0.8) warrant human oversight
...
Never ignore high-risk scores (&ge; 0.6) in production environments.
</Accordion>
...
<Warning>
Rate limit: 60 requests per minute per API key. For higher limits or batch processing, contact support@orcho.ai.
</Warning>